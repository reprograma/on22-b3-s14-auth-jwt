...
const findAllGames = async (req, res) => {
  try {
    ...
    if (!authHeader) {
      return res
        .status(401)
        .send("You've forgotten the authorization informations");
    }
    const token = authHeader.split(" ")[1];
    jwt.verify(token, SECRET, async function (error) {
      if (error) {
        return res.status(403).send("Access non authorized");
      }
...
const deleteGame = async (req, res) => {
  try {
    const authHeader = req.get("authorization");
    if (!authHeader) {
      return res.status(401).send("You've forgotten the token");
    }
    const token = authHeader.split(" ")[1];
    jwt.verify(token, SECRET, async function (error) {
      if (error) {
